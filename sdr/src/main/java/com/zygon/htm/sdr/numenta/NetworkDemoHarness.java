...
public class NetworkDemoHarness {
     ...
     *
     * @param map if called more than once to set up encoders for more than one
     * field, this should be the map itself returned from the first call to {@code #setupMap(Map, int, int, double,
     ...
     * @param periodic such as hours of the day or days of the week, which
     * repeat in cycles
     * @param clip whether the outliers should be clipped to the min and max
     * values
     * @param forced use the implied or explicitly stated ratio of w to n bits
     * rather than the "suggested" number
...
//        p.set(KEY.INFERRED_FIELDS, getInferredFieldsMap("consumption", SDRClassifier.class));
    ...
    /**
     * @return a Map that can be used as the value for a Parameter object's
     * KEY.INFERRED_FIELDS key, to classify the specified field with the
     * specified Classifier type.
     */
    ...
    /**
     * Returns the default parameters used for the "dayOfWeek" encoder and
     * algorithms.
     *
     * @return
     */
